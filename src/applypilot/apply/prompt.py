...
def build_prompt(job: dict, tailored_resume: str,
    ...
    if copilot:
        ...
        _copilot_handoff_rule = (
            ...
            "  HAND OFF IMMEDIATELY on this one. If the site wants you to CREATE AN ACCOUNT, "
            "verify an email or phone, set a password, or complete an SSO flow, you cannot do it "
            "and no amount of retrying will change that — stop on the FIRST screen that asks, "
            "before filling anything else. Do not try to guess a password, do not look for a "
            "'continue as guest' path more than once, and do not keep exploring the site. A "
            "handoff in 20 seconds is worth far more than a thorough one in 10 minutes: the "
            "human signs up once and every later application to this employer is already "
            "authenticated.\n"
