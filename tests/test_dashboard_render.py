...
@pytest.mark.skipif(not shutil.which("node"), reason="node not available")
def test_the_invite_tab_renders_a_form_and_only_where_there_is_an_address(tmp_path):
    """📅 Invite sends a real `.ics`, so it rides on the EMAIL address and nothing else.

    Driven through `contactPanel` rather than by calling `inviteChannel` directly: a pane that
    renders perfectly and is wired to no tab is the §Lessons 94 shape, where thirty-six passing
    tests called the editor's functions and none of them drove the render.
    """
    ok = _contact(id="i1", full_name="Dana Okafor", email="dana@ridgeline.test")
    ok["_pick"] = "invite"
    # Already invited once: the pane has to say that re-sending MOVES the meeting, or the
    # operator sends a second time expecting a correction and creates a duplicate.
    again = _contact(id="i2", full_name="Repeat", email="r@x.test")
    again["_pick"] = "invite"
    # A second, UNRELATED interaction, so "have we invited them" cannot pass by matching any row
    # at all — with only an invite in the list, a filter on `true` is indistinguishable from the
    # right one.
    again["interactions"] = [{"kind": "invited", "at": "2026-08-01T15:00:00+00:00",
                              "detail": "Intro call (30 min)", "source": "manual"},
                             {"kind": "deck", "at": "2026-08-02T09:00:00+00:00",
                              "detail": "", "source": "detected"}]
    # Never invited, but they HAVE done something — the case that separates the two filters.
    other = _contact(id="i4", full_name="Deck Only", email="d@x.test")
    other["_pick"] = "invite"
    other["interactions"] = [{"kind": "deck", "at": "2026-08-02T09:00:00+00:00",
                              "detail": "", "source": "detected"}]
    # No address: the tab must fall through to the box that adds one, not to an invite form.
    none = _contact(id="i3", full_name="No Address", email="", linkedin_url="")
    none["_pick"] = "invite"

    script = tmp_path / "invitetab.mjs"
    script.write_text(
        _STUBS
        + f"const SRC = {json.dumps(_page_js())};\n"
        + f"const CASES = {json.dumps({'ok': ok, 'again': again, 'none': none, 'other': other})};\n"
        + _TABS_DRIVER
    )
    proc = subprocess.run(["node", str(script)], capture_output=True, text=True, timeout=60)
    assert proc.returncode == 0, f"node failed:\n{proc.stderr[:2000]}"
    ...
    assert "📅 Invite" in out["ok"], "the invite tab is not on the strip"
    for control in ("inv-date", "inv-time", "inv-mins", "sendInvite"):
        assert control in out["ok"], f"the invite pane is missing {control}"
    # The recipient travels as a DATA ATTRIBUTE. Scraping it out of the meta row's first <a>
    # would confirm the wrong address the moment anything is added above it — on the one action
    # in this panel that a second click cannot undo.
    assert 'data-to="dana@ridgeline.test"' in out["ok"]

    # A previous invite is stated, and so is what a second one DOES.
    assert "Already invited" in out["again"]
    assert "moves that meeting" in out["again"], \
        "nothing says a re-send updates rather than duplicates"
    assert "Already invited" not in out["ok"], "claimed a previous invite that never happened"
    # Someone with a deck open and no invite must NOT read as invited: `lastInvite` has to
    # filter on the KIND, and a fixture holding only invites cannot tell the two apart.
    assert "Already invited" not in out["other"], "an unrelated interaction counted as an invite"
    assert "📅 Invite ✓" in out["again"]
    assert "📅 Invite ✓" not in out["ok"] and "📅 Invite ✓" not in out["other"]

    # Not replied yet: advise, never refuse (§Lessons 69). The button must still be there.
    assert "has not replied yet" in out["ok"]
    assert "sendInvite" in out["ok"], "the advisory disabled the send"

    # No address at all -> the add-an-email box, not an invite form aimed at nobody.
    assert "inv-date" not in out["none"], "offered an invite form with no address to send it to"
    assert 'data-field="email"' in out["none"]
    # ...and the tab says so from the strip, like the other two empty channels.
    assert "📅 Invite ＋" in out["none"], "an invite tab with no address is not marked empty"
...
@pytest.mark.skipif(not shutil.which("node"), reason="node not available")
def test_sending_an_invite_confirms_the_REAL_recipient_and_posts_the_form(tmp_path):
    """The one action in this panel a second click cannot undo, so it is driven end to end.

    Rendering the form proves nothing about what the button sends. The recipient in particular
    is §Lessons 29's shape — "the dangerous half of a feature is the half that looks identical
    when it is wrong": an invite addressed to the wrong person renders the same screen, logs the
    same success, and is only discovered by the person who receives it.

    "Are you sure?" is not a question anyone can answer, so the confirm has to NAME the address,
    the date, the time and the length.
    """
    script = tmp_path / "sendinvite.mjs"
    script.write_text(
        _STUBS
        + f"const SRC = {json.dumps(_page_js())};\n"
        + """
const seen = { confirm: '', posted: null, path: '' };
globalThis.confirm = (msg) => { seen.confirm = msg; return true; };
const F = (new Function(SRC + `
  post = async (p, payload) => { globalThis.__seen.path = p; globalThis.__seen.posted = payload;
                                 return { ok: true, organiser_link: 'https://cal/add' }; };
  return { sendInvite };`))();
globalThis.__seen = seen;

// The real DOM shape the handler walks: a .invite pane holding the fields, and a button inside.
const field = (cls, value) => ({ value, className: cls });
const fields = { '.inv-date': field('inv-date', '2026-09-01'),
                 '.inv-time': field('inv-time', '10:00'),
                 '.inv-mins': field('inv-mins', '45'),
                 '.inv-summary': field('inv-summary', 'Intro call'),
                 '.inv-body': field('inv-body', 'looking forward'),
                 '.inv-msg': { textContent: '', innerHTML: '' } };
const pane = {
  getAttribute: (k) => ({ 'data-cid': 'c9', 'data-to': 'dana@ridgeline.test' }[k]),
  querySelector: (sel) => fields[sel] || null,
  closest: () => null,
};
const btn = { disabled: false, textContent: 'Send invite', closest: (sel) => pane };
await F.sendInvite(btn);
console.log(JSON.stringify({ confirm: seen.confirm, posted: seen.posted, path: seen.path,
                             msg: fields['.inv-msg'].innerHTML }));
""", encoding="utf-8")
    proc = subprocess.run(["node", str(script)], capture_output=True, text=True, timeout=60)
    assert proc.returncode == 0, f"node failed:\n{proc.stderr[:2000]}"
    out = json.loads(proc.stdout.strip().splitlines()[-1])

    # The confirm names who, when and how long — read off the pane, not invented.
    assert "dana@ridgeline.test" in out["confirm"], f"the confirm does not name the recipient: {out['confirm']!r}"
    assert "2026-09-01" in out["confirm"] and "10:00" in out["confirm"]
    assert "45" in out["confirm"]

    # And what is POSTED matches what was confirmed. A confirm showing one thing while the
    # request carries another is worse than no confirm at all.
    p = out["posted"]
    assert out["path"] == "/api/contact/invite"
    assert p["contact_id"] == "c9"
    assert p["date"] == "2026-09-01" and p["time"] == "10:00"
    assert p["minutes"] == 45, "the length is not sent as a number"
    assert p["summary"] == "Intro call" and p["body"] == "looking forward"
    assert p["tz"], "no timezone was sent, so the server would have to guess one"

    # The sender's own calendar link is the whole reason this is not fire-and-forget.
    assert "https://cal/add" in out["msg"], "no way to add the meeting to your own calendar"
